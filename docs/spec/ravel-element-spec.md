...
A RavelElement is the base class for all Raveling custom elements. It can take three forms/modes:
...
3. Factory: Creates copies of itself 
...
4. Observable: Every element should have a popup custom dashboard for debugging and observability (set by a modal flag)
5. Dockable: Every element should be able to send a "dock me!" message to a ravel-dock and be minimized and captured by the dock. 
6. Virtual Events: Every element can potentially have listeners that respond to "virtual" versions of browser events (e.g. 'click', 'drag-begin', 'drag-end' etc.)
