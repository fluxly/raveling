...
/**
 * Runs the feral-vocaloid Pure Data patch via libpd (WebAssembly) on a live
 ...
 * | `patch-url`     | string  | `/core/libs/libpd-wasm/feral-vocaloid.pd`    | URL of the Pd patch to load         |
...
export class RavelGlottalizer extends RavelElement {
    ...
    private async _doInit(): Promise<void> {
        ...
        try {
            ...
            // ── Level meter (using a simple script-processor-style approach) ─
            ...
            // Blob URL module workers need fully-qualified URLs — absolute
            // paths like /libs/... don't resolve correctly from a blob origin.
    ...
    private _renderParams(p: typeof this._params): void {
        // Normalize each param to 0-1 for the bar width
        const vals = [
            (p.frequency - 80) / (1100 - 80),            // frequency
            p.tenseness,                                   // tenseness
            p.intensity,                                   // intensity
            p.loudness,                                    // loudness
            (p.tongue.index - 12) / (29 - 12),            // tongue index
            (p.tongue.diameter - 2.05) / (3.5 - 2.05),   // tongue diameter
        ];
        const labels = [
